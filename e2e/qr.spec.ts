...
test("anonym: URL eingeben erzeugt einen lesbaren QR-Code", async ({ page }) => {
  await page.goto(`${QR}/`);
  await page.getByLabel("Link oder Text").fill("https://example.org");
  await page.getByRole("button", { name: /erzeugen/i }).click();
  expect(await decodeQr(await readQrSvg(page))).toBe("https://example.org");
});

test("anonym: WLAN-Formular erzeugt den korrekten WIFI:-String", async ({ page }) => {
  await page.goto(`${QR}/wifi`);
  await page.getByLabel("SSID").fill("IuK-Test");
  await page.getByLabel("Passwort").fill("geheim");
  await page.getByRole("button", { name: /erzeugen/i }).click();
  expect(await decodeQr(await readQrSvg(page))).toBe("WIFI:T:WPA;S:IuK-Test;P:geheim;H:false;;");
});
...
test("Admin-Route ist ohne die Gruppe nicht vorhanden (404, nicht 403)", async ({ page }) => {
  await devLogin(page, { host: "qr.localtest.me", groups: "iuk-qr-user" });
...
test("der QR-URL-Vertrag funktioniert direkt", async ({ page }) => {
  ...
  await page.goto(`${QR}/qr?data=${encodeURIComponent("https://example.org")}&label=Test&kind=url`);
  expect(await decodeQr(await readQrSvg(page))).toBe("https://example.org");
});
...
test("anonym: Kontakt-Formular erzeugt eine lesbare vCard", async ({ page }) => {
  ...
  const email = "maximiliane.von.musterhausen@iuk-kreisverband-berlin-nordwest.example";
  const org =
...
const VIEW = `/qr?data=${encodeURIComponent("https://example.org")}&label=Test&kind=url`;
...
test("PNG speichern laedt eine Datei mit demselben Code herunter", async ({ page }) => {
  ...
  expect(await decodeQrPng(await readFile(path))).toBe("https://example.org");
});
...
test("Teilen reicht Nutzlast und Bezeichnung an das System weiter", async ({ page }) => {
  ...
  await expect
    ...
    .toEqual([{ title: "Test", text: "https://example.org" }]);
});
