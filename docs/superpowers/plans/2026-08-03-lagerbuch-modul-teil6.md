...
| 3 | Die Entprellung, die den Tastendruck als `?q=` in die URL schreibt, plus der `committedQ`-Tanz (`JournalFilter.tsx:29-36,44-52`) | **DOM:** `verwaltung/(arbeit)/journal/JournalFilter.test.tsx` (Teil 5, T147) — gefälschte Uhr, **ein** Schreibvorgang je Tipppause; `committedQ` selbst steht wörtlich in `JournalFilter.tsx:36`. Gestützt auf `_ui/filter.test.tsx` (T109), das die `useUrlFilter`-Schreibmechanik trägt, aber weder Debounce noch den `committedQ`-Tanz prüft. **E2E:** ⚠️ **kein Nachfolger gefunden** — `e2e/lagerbuch-verwaltung.spec.ts` (T150) trägt ausschließlich die vier Modulnavigations-Zusicherungen (Teil-5-Plan, Welle 7, T150-Brief); T150s eigener Scope nennt `_lib/nav.ts`/`.modulnav`, nicht die Journal-Suche. Die alte `suche-filter.spec.ts:30` (`toHaveURL(/[?&]q=Verband/)`) hat keinen Nachfolger in irgendeiner `e2e/*.spec.ts` (T174-Befund, s. Bericht) | 2 | ✅ DOM · ❌ E2E |
...
| ☐ | `suche-filter.spec.ts` | **Umschreiben**, Netz zuerst (§12.1 Punkte 2 und 3), mit der Rollen-Gegenprobe aus §12.3 Regel 2. Die literale URL-Zusicherung `?q=Verband` **bleibt** — sie ist der einzige Beleg für den URL-Vertrag. ⚠️ **T174-Befund:** die DOM-Hälfte lebt in `journal/JournalFilter.test.tsx` (T147), nicht in `_ui/filter.test.tsx` (T109, nur die `useUrlFilter`-Mechanik). Die **E2E-Hälfte hat keinen Nachfolger** — `?q=Verband` steht in keiner `e2e/*.spec.ts` | `journal/JournalFilter.test.tsx` (T147) + `_ui/filter.test.tsx` (T109) + ⚠️ E2E fehlt |
