...
- **Bedienelemente setzen kein `size`** — ohne Ausnahme. Auf `FullShell`-Seiten (und `aufgaben` ist eine) ist `controlHeight` 44 und schon das richtige Touchmaß; `size="large"` wäre 72px, `size="small"` 24px und damit unter der Mindesttapfläche. Die frühere Ausnahme „`size="small"` in Tabellenzeilen" ist mit den 56px gefallen (`docs/design/README.md`, Falle 4); `_ui/SeitenKopf.test.tsx` meldet beide Werte.
