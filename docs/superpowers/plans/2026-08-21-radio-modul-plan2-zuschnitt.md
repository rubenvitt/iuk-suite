...
| ⛔ **Merge-Bedingung: der Rollout der Kachel „Funkgeräte" bis Planteil 3** | Entscheidung des Betreibers, **vor** dem Merge nach `main` | Der Merge ist kein Feature-Flag: `registry.ts:192-194` setzt `showInSwitcher: true` und `requiresAuth: false` — die Kachel „Funkgeräte" erscheint danach für **jeden**, auch anonym, und führt bis Planteil 3 auf **404**, weil unter `src/app/m/radio/` keine `page.tsx` liegt. Auf `main` steht ein **automatischer Rollout** (`.github/workflows/ci.yml:309-319`: Job `deploy`, `if: github.ref == 'refs/heads/main' && vars.SUITE_STACK_DIR != ''`, `runs-on: [self-hosted, iuk-suite-prod]`), gebremst nur durch `environment: produktion` mit Reviewer-Klick und die Existenz von `vars.SUITE_STACK_DIR`. **Zwei Auswege:** entweder Planteil 2 und 3 zusammen ausliefern, oder nach dem Merge den Rollout auf `main` bis Planteil 3 **nicht** freigeben. ⚠️ Der naheliegende dritte Weg — `showInSwitcher: false` bis Planteil 3 — ist **keiner**: er widerspräche Betreiberentscheidung 5 und machte `registry.test.ts:97` rot (`expect(anon).toEqual(["qr", "radio"])`) |
...
lässt `admin/(druck)/layout.tsx` bei `requireRadioAdmin()`, Spec:4369-4378 verteilt die zehn Seiten
einzeln. ⛔ **Drei der zehn bleiben auf `requireRadioAdmin()`** — `admin/(arbeit)/versionen`
(Spec:4376), `admin/(arbeit)/zugaenge` (Spec:4377) und `admin/(druck)/zugaenge/blatt` (Spec:4378);
sie tragen die Zugangscodes. Wer nur `Spec:4369-4375` liest, setzt `requireRadioVerwaltung()` auf
alle zehn und senkt genau diese drei Flächen ab. ⛔ **Bis dahin sperrt das `(arbeit)`-Layout jede
Updater-Person mit 404, bevor irgendeine Seite läuft** — und typecheck, lint und build bleiben
grün. `riegel.test.ts` Klausel (a) ist bereits pfadsensitiv gebaut und lässt beide Namen im
Arbeitszweig zu; der Wechsel macht den Scan **nicht** rot. ⛔ **Und das ist keine Lücke im
Testkopf, sondern strukturell** — Klausel (a) und (e) lassen im `(arbeit)`-Zweig
`requireRadioAdmin(` **oder** `requireRadioVerwaltung(` zu, und zwar **absichtlich**
(`riegel.test.ts:373-384`): ohne das ODER wäre der Scan gegen `Spec:4367` rot-by-construction,
sobald dieser Wechsel steht. Ein pfadsensitiver Scan kann innerhalb **derselben** Route-Group
„richtig auf der Verwaltungs-Stufe" nicht von „fälschlich von der Admin-Stufe abgesenkt"
unterscheiden — betroffen sind **zwei** Seiten, `admin/(arbeit)/versionen` und
`admin/(arbeit)/zugaenge`; nur `admin/(druck)/zugaenge/blatt` fällt in den strengen Zweig und wäre
gedeckt. **Für diese zwei Seiten ist dieser Plantext die einzige Sperre.** ⛔ Die zweite Stufe
kommt als **zweite Funktion**, nie als `||` in `istRadioAdmin` — `zugang.test.ts` hält diese eine
Richtung fest. ⬜ **E1b** (der Gruppenname) ist vorher fällig,
...
| 3 | `requireRadioAdmin` aus `_lib/zugang.ts` ist der Verwaltungsriegel **für Seiten, Layouts und Server Actions** — in Planteil 2 der **einzige**. Er ruft `requireRadioHost` **intern**, als erste Anweisung. ⛔ **Verwaltungs-Route-Handler unter `admin/` NICHT** — sie riegeln mit `radioHostOderNull` + `istRadioAdmin(await viewerOderNull())` und bauen ihre 404 selbst (**B11**, Spec:100/2778/4379, bestätigt B17 Spec:117); ein `requireRadioAdmin` dort erzeugt den Login-Umweg, den B11 abgeschafft hat. `riegel.test.ts` Klausel (c) prüft es. ⚠️ **Ab Planteil 4 gilt der Wortlaut „jede Server Action ruft ihn" nicht mehr**: `requireRadioAdmin` ist dann die **strengere von zwei** Verwaltungsstufen; welche Fläche welche nimmt, steht in `Spec:4367-4380` und in **NS-Z8** | **4** |
...
| **Ist die zweite Rechtestufe vorgesehen, ohne gebaut zu werden?** | Ja, und die **Riegelform** sieht sie vor, nicht nur die Prosa — das ist die Auflage aus `KONTEXT-radio-planteil2.md:30-31`. **Fünf Träger, nicht vier — die Layout-Achse allein trägt die Umstellung nicht:** (1) `riegel.test.ts` Klausel (a) ist **pfadsensitiv** — `(arbeit)` lässt `requireRadioAdmin(` **oder** `requireRadioVerwaltung(` zu (Spec:4367), `(druck)` nur den ersten (Spec:4368); ohne das wäre der Scan gegen die verbindliche Bauform rot-by-construction. (2) `_lib/zugang.ts` benennt die Trennung, die die Spec macht: der **Riegel** beider Stufen gehört in **diese** Datei (Spec:4287-4288), die **Gruppenquelle** und die Feld-Allowlist in `_lib/rollen.ts` mit eigenem Test (Spec:4420-4422). (3) `zugang.test.ts` verriegelt die eine Richtung, in der Planteil 4 sie versehentlich zur **Aufweichung** machen würde — mit einer Gegenprobe, die den roten Zustand herstellt (Z4, Schritt 3). (4) **NS-Z8** nennt den Wechsel in `admin/(arbeit)/layout.tsx` als ⛔-Auflage samt Preis: bis dahin sperrt das Layout jede Updater-Person mit 404, still. (5) **NS-Z8 nennt auch die Seiten-Achse**, und dort trägt kein Test: die drei Seiten `versionen`, `zugaenge` und `zugaenge/blatt` bleiben auf `requireRadioAdmin()` (Spec:4376-4378), und `riegel.test.ts` kann eine falsche Absenkung von `admin/(arbeit)/versionen` oder `admin/(arbeit)/zugaenge` auf `requireRadioVerwaltung()` **strukturell nicht** fangen — Klausel (a)/(e) lassen im `(arbeit)`-Zweig beide Namen absichtlich zu (`riegel.test.ts:373-384`). Nur `admin/(druck)/zugaenge/blatt` fällt in den strengen Zweig und wäre gedeckt |
