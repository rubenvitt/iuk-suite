...
| **`data-person="<id>"` plus `<section aria-labelledby>` je Person** | **übernommen** | Sobald „Auslastung diese Woche" je Person mehr als eine Zahl zeigt, ist das die Adressierung, die `data-rolle` allein nicht leisten kann. Kostet nichts und macht die Zone bedienbar für Screenreader. |
...
| **Die Tafel (dreimal `Wochenplan`) — auf `/verteilen` statt auf `/`** | **nicht übernommen** | `/verteilen` ist der **Stapelplatz** (§4.4) und muss die leichteste Seite des Moduls bleiben; drei gestapelte Wochenachsen machen sie zur schwersten. Und sie machten `data-rolle` von einer Identität zu einer Klasse — ein Preis, den B selbst ausschreibt. Was Rike vor der Entscheidung braucht, ist die **Zahl**, und die trägt „Auslastung diese Woche" auf `/`; die Feinverteilung entscheidet sie im Modal, das die Auslastung nach jeder Zuweisung neu rechnet. |
...
| Tests 32–34 Umschaltung 390/820/1280 (`tagesliste` ⇄ `wochengitter`) | unberührt | `/` der BuFDi trägt weiterhin genau **eine** Achse. „Auslastung diese Woche" ist **keine** Achse, sondern eine Zahlen-Zone (§5.2) — `data-rolle` bleibt damit eine Identität. |
...
3  Die Flaeche der Rolle   BuFDi -> die Woche · Koordination -> „Auslastung diese Woche"
...
- **R3 — Zonen sind alle Anlässe ab Position 2 in `anlaesse`, plus der Anlass auf Position 1 genau
  ...
  **Welche Anlässe „bereits die Fläche der Rolle" sind, ist keine Auslegungsfrage, sondern eine
  ...
  | **Koordination** | **keiner** | „Auslastung diese Woche" zeigt **Zahlen je Person**, keine Aufgabenzeilen. Es gibt dort nichts, was eine Zone wiederholen könnte; alle sechs Sprossen können eine Zone bilden. |
...
- **D — Ein Deckel setzt einen Ausgang voraus.** Eine **Zone** (Ebene 4) **mit Sammelziel** zeigt
  ...
  **Die Fläche der Rolle (Ebene 3) ist aus demselben Grund ausgenommen**, und das ist keine
  ...
  Aufträge", „Auslastung diese Woche" und die Wochenachse sind kein Vorrat, sondern die Fläche selbst.
...
| **Koordination** | alles, was nicht überfällig, nicht `eingegangen`, nicht `freigabe_offen`, nicht `zurueckgewiesen` und bei einer aktiven Person liegt | als **Zahl je Person** in „Auslastung diese Woche" (`<n> Aufgaben`), und über „Zeitplan ansehen" → `/plan/<id>` mit jeder Zeile |
...
**Ihre Fläche der Rolle ist nicht der Posteingang, sondern „Auslastung diese Woche".** Der Posteingang
...
Auslastung diese Woche                                        KW 34 · Mo–Fr
┌ ALINA ───────────────┬ BENDIX ──────────────┬ CARLA ───────────────┐
...
**Auf 360px bleibt „Auslastung diese Woche" vollständig** — eine Zeile je Person mit **Wochenwert**,
...
| **U-7** | R3, „welcher Anlass ist bereits die Fläche der Rolle" | **Behoben.** §3.4 bekommt eine **Ausnahmetabelle je Rolle** mit dem Maßstab davor: BuFDi → Rang 3/4/5 (und die Abhängigkeit von Regel V ist mitgeschrieben) · Koordination → **keine** Ausnahme, weil „Auslastung diese Woche" nur Zahlen zeigt · Auftrag → **alle**, Ebene 4 existiert für diese Rolle nicht. §5.3 sagt das jetzt auch im Fließtext. |
...
## 11.5 Nachtrag 2026-08-16: die Gestaltrunde am Koordinationseinstieg

Diese Spec ist **inhaltlich unverändert gültig** — was wo steht, welche Sprosse führt, welches
Prädikat entscheidet. Der Betreiber hat nach der Umsetzung über die **Gestalt** geurteilt („die UI
wirkt nicht ausgereift … eher wie eine alte Formularanwendung … mehr Diversität im UI/UX"), und die
folgenden Punkte sind daraufhin geändert worden. Sie stehen hier, weil eine Spec, die etwas anderes
beschreibt als die Fläche, schlimmer ist als keine.

| Was die Spec sagte | Was jetzt gilt | Warum |
|---|---|---|
| §3.6: die Zeile ist eine Flex-Reihe aus sieben Geschwistern | **Raster.** `.zeilenListe` trägt `grid-template-columns`, die Zeile übernimmt es per `subgrid`; drei inhaltsunabhängig bemessene Spuren (`minmax(140px,1fr)` / `minmax(0,1.6fr)` / `minmax(150px,auto)`), Zellen immer alle drei, auch leer | Eine Flex-Zeile kennt nur ihre eigenen Kinder — Ausrichtung über Zeilen hinweg kann sie strukturell nicht. Die vier „Anders zuweisen"-Knöpfe standen deshalb auf vier X-Positionen. **Inhaltsunabhängig** bemessen, weil `auto`-Spuren jeder Zone eine eigene Geometrie geben. |
| §6.5: „es kommt **keine** neue `--auf-*`-Variable dazu" | **`--auf-karte`** als dritte, bedeutungsfreie Trägerfläche | s. die durchgestrichene Zeile in §8. |
| §6.5: „genau zwei neue Regeln" in der Medienabfrage | vier | Ein Raster braucht unter 768px Spurenzahl, Zellenausrichtung und leere Zellen umgeschaltet, plus die Aktionsspalte, die auf Touch nicht an `:hover` hängen darf. **Unverändert gilt die Zahl, an der ein Tor hängt: genau EINE Medienabfrage.** |
| §5.2: die Zone heißt „Die Woche der drei" | **„Auslastung diese Woche"** | Die alte Überschrift war an die heutige Zahl der BuFDis gebunden und beim vierten schlicht falsch — sie wird nicht rot, sie wird unwahr, und kein Tor der Suite kann das sehen. Der neue Name skaliert mit jeder Personenzahl und sagt, was die Zone zeigt: eine Menge im Verhältnis zu einer Kapazität. Die bildhaften Namen dieses Repos sind Namen von **Entwürfen**, nicht Überschriften auf der Fläche. |
| §5.2: die Zone zeigt Wochenwert und überbuchte Tage als **Textzeilen** | **Balken** — ein Wochenbalken je Person plus fünf Tagesstreifen; Skala `max(soll, verplant)` mit Kapazitätsmarke | Ein Verhältnis liest man aus einer Länge schneller als aus zwei Zahlen, und die Überbuchung war die einzige Auffälligkeit der Kachel und stand als Fließtext zwischen Fließtexten. Die Tagesstreifen beantworten, was die Wochensumme nicht kann: 6 von 39 Wochenstunden **und** Montag doppelt verplant. Farbe bleibt die zweite Schicht (§9.3): stahlgrau, bis die Kapazität reißt; das Wort steht weiter daneben. |
| §7 Nr. 3: „Anders zuweisen" ist in der Zeile ein Knopf, der ein Modal öffnet | **Inline** (`_ui/ZuweisenInline.tsx`): eine aufklappende Namensliste mit Wochenauslastung, der Klick auf den Namen **ist** das Absenden | Für eine Zuweisung besteht die Entscheidung aus genau einer Angabe; Knopf → Modal → Formular → Absenden sind vier Schritte für einen. **Die Bestätigung bleibt** — der Satz über den geleerten Zeitplan steht als erste Zeile über der Liste, also weiterhin zwischen Absicht und Absenden. Der Modalweg bleibt in der Führungskarte und auf `/a/<id>`, wo er zusätzlich den optionalen Zeitvorschlag trägt. |
| §3.4/§3.5: jede Zone ist eine Liste mit Überschrift | Die **Form** folgt dem Zweck: dichtes Raster, wo man vergleicht; knappe Zeile, wo man nur wissen will, **dass** es sie gibt | „Mehr Diversität": fünf gestapelte Listen sind derselbe monotone Eindruck wie fehlende Spalten. **Die Informationsarchitektur ändert sich dabei nicht** — dieselben Angaben in derselben Reihenfolge (§10 Prüffrage 7), nur anders gesetzt. |
