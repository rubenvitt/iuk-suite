...
/** Success means a prepared response, including streams; receipt by the client is unobservable. */
  ...
  return withAuditContext({ actor }, async () => {
    try {
      ...
      if (response.ok || response.status === 401 || response.status === 403 || response.status >= 500) {
        auditEvent({ module, action, objectType, origin: "server", result: response.ok ? "success" : response.status < 500 ? "denied" : "failure" });
      }
      return response;
    } catch (error) {
      // Authorization redirects/notFound are handled by the existing guard before this boundary.
      auditEvent({ module, action, objectType, origin: "server", result: "failure" });
      throw error;
    }
  });
}
