...
/**
 * AUCH DIE KOORDINATION AENDERT KEINE FREMDEN PLAENE. Sarah *schlaegt vor* (`vorschlag_datum`),
 * sie setzt nicht (`plan_datum`) — die Gestaltungshoheit ueber den eigenen Tag liegt beim BuFDi
 * (Anforderung 3 des Auftraggebers). Also ausschliesslich die Zielperson selbst, und aktiv.
 */
 ...
 * `rolle === "koordination"` je fuer sich, und Sarah bekaeme einen Freigabeknopf fuer Leas eigene
 * Aufgabe — die gar keine Freigabestufe hat (Spec §5.2: Selbstaufgaben gehen `in_arbeit` →
 * `abgeschlossen`, ohne `freigabe_offen`).
 *
...
/**
 * FUER ALLE WAHR. BuFDis sehen die Zeitplaene der anderen lesend — Vertretungsabsprachen ohne
 * Sarah als Nadeloehr —, `koordination` und `auftrag` sehen ohnehin alle. Kein `istAktiv`: ein
 * ausgeschiedener BuFDi liest weiterhin, was war.
 *
...
export function istVertretungsfreigabe(p: PersonRow, a: AufgabeRow): boolean {
  return p.rolle === "koordination" && p.id !== a.prueferId;
}
