...
describe("NeuPage — Default-Export: keine Rollenschranke, nur die Erklaerseite fehlt", () => {
   ...
   * bewusst (s. Kopfkommentar `page.tsx`) und hier fuer alle drei Rollen UND eine ausgeschiedene
   * Person als Vertrag festgehalten.
   */
  it.each<Rolle>(["auftrag", "koordination", "bufdi"])(
    "Rolle %s bekommt die Seite (200), kein notFound()",
    async (rolle) => {
      const p = legePerson(`np-${rolle}`, rolle);
