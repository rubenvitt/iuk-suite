 ...
 *       entleiherVorschlaege, listeAktualisieren (briefs/A17.md:26-29)  -> 9  ERLEDIGT
 *
 * ⚠️ NUR DIE FUNKTIONEN ZAEHLEN. `gate.ts` fuehrt `export type GateZustand` (Spec:2344),
 * `sitzung.ts` `export type ErneuerungErgebnis` und `ausleihe.ts` den E11-Re-Export der
 * zwei Ergebnistypen — alle drei loescht der Uebersetzer; sie heben diese Zahl nicht.
 */
const ACTION_DEKLARATIONEN_ANZAHL = 9;
...
describe("radio-_actions: jede exportierte Action traegt ihren Riegel", () => {
  it("die Dateizahl steht EXAKT auf dem Stand dieses Planteils", () => {
     ...
     *   A17 legt `ausleihe.ts` an                    -> 4   ERLEDIGT (Planteil 3)
     */
