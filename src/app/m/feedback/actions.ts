...
export async function freigebenAction(formData: FormData): Promise<void> {
  ...
  return withAuditContext({ actor: auditActor(viewer) }, async (): Promise<void> => {
    ...
    /*
     * ⚠️ DIE BEIDEN ZEITRIEGEL STEHEN IN `freigabelage` (`_lib/lifecycle.ts`),
     * nicht hier: dieselbe Frage beantwortet die Oberfläche noch einmal, um den
     * Knopf gar nicht erst anzubieten. Zwei Fassungen liefen auseinander, und
     * der sichtbare Teil wäre der falsche gewesen. Die Begründung beider
     * Ausgänge steht an der Funktion.
     */
    const lage = freigabelage(eve.date, hours, jetzt);
    if (lage === "zuFrueh") throw new Error("Freigabe erst am Tag des Dienstabends");
    if (lage === "abgelaufen") throw new Error("Die Frist dieses Abends ist bereits abgelaufen");
    releaseSurveyForEvening(db, { eveningId, closeAfterHours: hours, now: jetzt });
    revalidate();
  });
}
