    ...
    async (): Promise<HelferErgebnis<{ ausgesondert: number }>> => {
      ...
      try {
        fachFehler = db.transaction((tx): string | null => {
          ...
          /*
           * DIE GEMELDETE VERFALLSANGABE DER BOX (DRK-377) — sie faellt mit dem
           * letzten Stueck, wie beim Einraeumen seit DRK-404. Eine Frage nach
           * dem Datum auf der Packung braucht es hier nicht: das Material geht
           * in den Muell, kein Datum muss es in einen Schrank begleiten.
           */
          raeumeVerfallAmLeerenOrt(tx, ENTNAHMEBOX_ID, v.artikelId);
