...
describe("einraeumPosten — dieselbe Faltung, zwei Felder mehr (DRK-381)", () => {
  it("haengt Fach und Stilllegung an, ohne die Posten selbst zu veraendern", () => {
     ...
     *
     * ⚠️ DIESE LISTE IST VON HAND GEFUEHRT UND MUSS MITWACHSEN. `gemeldet` kam
     * mit DRK-377 an `BoxPosten` dazu; fehlte es hier, verglichen die beiden
     * Seiten ein Feld weniger, als die Faltung liefert — der Test bliebe gruen
     * und seine Zusage („dieselbe Faltung") waere still nur noch halb wahr.
     * Genau das hat er beim Merge der beiden Tickets gemeldet.
     */
    const nurBoxPosten = reich.map((p) => ({
      artikelId: p.artikelId, artikelName: p.artikelName, einheit: p.einheit,
      menge: p.menge, chargen: p.chargen, gemeldet: p.gemeldet,
    }));
