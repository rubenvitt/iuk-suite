...
import { angegebenZuGross, begrenztesJson } from "../../_lib/begrenztesJson";
...
export async function POST(req: Request) {
  ...
  if (angegebenZuGross(req, SYNC_MAX_BODY_BYTES)) return fehler(413, "body_too_large", "Sync-Body ist zu groß");
  const db = getDb();
  ...
  const body = await begrenztesJson(req, SYNC_MAX_BODY_BYTES, "Sync-Body ist zu groß");
